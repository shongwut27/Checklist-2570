...
const target = `    };
    ...
    let generatedPdfUrl = "https://drive.google.com/drive/folders/1uCP6AOi-d2JhHFsouZJhnu0_zNZquTSj";
    if (targetWebhookUrl) {
      try {
        ...
        if (gasJson && gasJson.pdfUrl) {
          generatedPdfUrl = gasJson.pdfUrl;
        }
      } catch (e) {
    ...
    newRecord.pdfUrl = generatedPdfUrl;
    dbRecords.push(newRecord);
    saveRecords(dbRecords);`;

const replacement = `    };

    newRecord.pdfUrl = "https://drive.google.com/drive/folders/1uCP6AOi-d2JhHFsouZJhnu0_zNZquTSj";
    dbRecords.push(newRecord);
    saveRecords(dbRecords);
...
console.log(code.includes(replacement) ? "Success" : "Failed to replace");
